...
import { suspendUser, UserServiceError } from "@/services/user-service";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { return apiOk(await suspendUser((await params).id, parsed.data.reason, actor, context), { requestId: context.requestId }); }
  catch (error) { return handle(error, context.requestId); }
}
function handle(error: unknown, requestId: string) { return error instanceof UserServiceError ? apiError(error.code, error.message, { status: error.status, requestId }) : apiError("USER_SUSPEND_FAILED", "Unable to suspend account.", { status: 500, requestId }); }
